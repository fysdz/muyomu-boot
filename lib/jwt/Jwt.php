...
use boot\lib\View;
use function boot\lib\getErrorDescription;
...
class Jwt
{
    ...
    public function verifyToken(string $Token):array
    {
        ...
        if (count($tokens) != 3)
            View::jsonReturn(getErrorDescription("b-4-0"));
        ...
        if (empty($base64decodeheader['alg']))
            View::jsonReturn(getErrorDescription("b-4-1"));

        //签名验证
        if (self::signature($base64header . '.' . $base64payload, $this->key, $base64decodeheader['alg']) !== $sign)
            View::jsonReturn(getErrorDescription("b-4-2"));

        $payload = json_decode(self::base64UrlDecode($base64payload), JSON_OBJECT_AS_ARRAY);

        //签发时间大于当前服务器时间验证失败
        if (isset($payload['iat']) && $payload['iat'] > time())
            View::jsonReturn(getErrorDescription("b-4-3"));

        //过期时间小宇当前服务器时间验证失败
        if (isset($payload['exp']) && $payload['exp'] < time())
            View::jsonReturn(getErrorDescription("b-4-4"));

        //该nbf时间之前不接收处理该Token
        if (isset($payload['nbf']) && $payload['nbf'] > time())
            View::jsonReturn(getErrorDescription("b-4-5"));
